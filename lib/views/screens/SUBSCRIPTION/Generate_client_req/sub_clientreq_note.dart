...
class sub_clientreqNote extends StatefulWidget {
  const sub_clientreqNote({super.key});

  @override
  State<sub_clientreqNote> createState() => _sub_clientreqNoteState();
}

class _sub_clientreqNoteState extends State<sub_clientreqNote> {
  ...
  void _addtable_row() {
    ...
    setState(
      () {
        sub_clientreq_recommendationList.add({
  ...
  void _updatenote() {
    if (_noteformKey.currentState?.validate() ?? false) {
      setState(
        () {
          sub_clientreq_noteList[noteeditIndex!] = {
            // 'notetype': selectedheadingType ?? '',
            // 'noteheading': selectedheadingType == 'With Heading' ? NoteHeadingController.text : '',
          ...
          _clearnoteFields();
          noteeditIndex = null;
          notelength = sub_clientreq_noteList.length;
        },
      );
    }
  }
  ...
  void _updatetable() {
    // if (_notetable_formKey.currentState?.validate() ?? false) {
    sub_clientreq_recommendationList[notetable_editIndex!] = {
      // 'notetype': selectedheadingType ?? '',
      // 'noteheading': selectedheadingType == 'With Heading' ? NoteHeadingController.text : '',
  ...
  void _editnote(int index) {
    Map<String, dynamic> note = sub_clientreq_noteList[index];
    setState(
      () {
  ...
  void _editnotetable(int index) {
    Map<String, dynamic> note = sub_clientreq_recommendationList[index];
    setState(
      () {
        // NoteHeadingController.text = note['noteheading'] ?? '';
  ...
  Widget sub_clientreq_noteLists() {
    return ListView.builder(
        itemCount: sub_clientreq_noteList.length,
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              Text(
                                '${sub_clientreq_noteList[index]['notecontent']}', // Display camera type from map
                    ...
                    IconButton(
                      onPressed: () {
                        setState(() {
                          sub_clientreq_noteList.removeAt(index);
                          notelength = sub_clientreq_noteList.length;
                        });
                      },
                      icon: const Icon(
  ...
  Widget noteTable() {
    return ListView.builder(
        itemCount: sub_clientreq_recommendationList.length,
        itemBuilder: (context, index) {
          return GestureDetector(
            onTap: () {
                    ...
                    Expanded(
                      child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              Text(
                                '${sub_clientreq_recommendationList[index]['key']}', // Display camera type from map
                                style: const TextStyle(color: Primary_colors.Color1, fontSize: 10),
                              ),
                            ],
                          )),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            // const SizedBox(height: 10),
            if (sub_clientreq_noteList.isNotEmpty)
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
            ...
            const SizedBox(height: 10),
            if (sub_clientreq_recommendationList.isNotEmpty)
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
              ...
              child: Row(
                ...
                children: [
                  Button1(
                    ...
                    onPressed: () {
                      noteeditIndex == null ? Generate_sub_clientreq.backTab() : _resetEditingStateNote(); // Reset editing state when going back
                    },
                  ),
                  ...
                  if (sub_clientreq_noteList.isNotEmpty || sub_clientreq_recommendationList.isNotEmpty)
                    Button1(
                      ...
                      text: 'Submit',
                      onPressed: () async {
                        if (sub_clientreq_client_addr_name.isNotEmpty && sub_clientreq_client_addr.isNotEmpty && sub_clientreq_bill_addr_name.isNotEmpty && sub_clientreq_bill_addr.isNotEmpty) {
                          final pdfData = await generate_sub_clientreq(PdfPageFormat.a4, sub_clientreq_products, sub_clientreq_client_addr_name, sub_clientreq_client_addr, sub_clientreq_bill_addr_name, sub_clientreq_bill_addr, sub_clientreq_no, sub_clientreqDetailsState.pickedFile != null ? sub_clientreqDetailsState.pickedFile?.files.single.path : "");
                          const filePath = 'E://Client_requirement.pdf';
                          final file = File(filePath);
                          await file.writeAsBytes(pdfData);

                          Subscription_Client.sub_clientreq_Callback();
