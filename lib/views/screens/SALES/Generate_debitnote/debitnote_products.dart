...
class debitnoteProducts extends StatefulWidget {
  const debitnoteProducts({super.key});

  @override
  State<debitnoteProducts> createState() => _debitnoteProductsState();
}

class _debitnoteProductsState extends State<debitnoteProducts> {
  int? editIndex1;
  ...
  void _addproduct() {
    if (formKey1.currentState?.validate() ?? false) {
      ...
      bool exists = debitnote_productDetails.any((product) => product['productName'] == productNameController.text && product['remarks'] == remarksController.text && product['hsn'] == hsnController.text && product['price'] == double.parse(priceController.text) && product['quantity'] == int.parse(quantityController.text) && product['gst'] == double.parse(GSTController.text));
      ...
      setState(() {
        ...
        _clearFields();
        // print(debitnote_productDetails.length);
      });
    }
  }
  ...
  void _updateproduct() {
    if (formKey1.currentState?.validate() ?? false) {
      setState(
        () {
          debitnote_productDetails[editIndex1!] = {
            'productName': productNameController.text,
            'remarks': remarksController.text,
            'hsn': hsnController.text,
            'price': double.parse(priceController.text),
  ...
  void _editproduct(int index) {
    Map<String, dynamic> product = debitnote_productDetails[index];
    setState(
      () {
  ...
  Widget debitnote_productDetailss() {
    return Column(
      mainAxisAlignment: MainAxisAlignment.start,
      crossAxisAlignment: CrossAxisAlignment.center,
      children: [
        for (int i = 0; i < debitnote_productDetails.length; i++)
          Column(
            mainAxisAlignment: MainAxisAlignment.start,
                        ...
                        SizedBox(
                          ...
                          child: Padding(
                            ...
                            child: Text(
                              ...
                              '${i + 1}. ${debitnote_productDetails[i]['productName']}', // Display camera type from map
                        ...
                        Row(
                          children: [
                            IconButton(
                              onPressed: () {
                                setState(() {
                                  debitnote_productDetails.removeAt(i);
                                });
                              },
                              icon: const Icon(
                                Icons.close,
          ...
          child: Form(
            ...
            child: Row(
              ...
              children: [
                    ...
                    Row(
                      ...
                      children: [
                        Button1(
                          ...
                          onPressed: () {
                            editIndex1 == null ? Generate_debitnote.backTab() : _resetEditingState(); // Reset editing state when going back
                          },
                        ),
                ...
                if (debitnote_productDetails.isNotEmpty)
                  Column(
                    children: [
                      const SizedBox(height: 25),
                      Container(
                                ...
                                child: Padding(
                                  ...
                                  child: SingleChildScrollView(
                                    child: Padding(
                                      ...
                                      child: debitnote_productDetailss(),
                                    ),
                                  ),
                                ),
                              ),
                      ...
                      if (debitnote_productDetails.isNotEmpty)
                        Button1(
                          ...
                          onPressed: () {
                            ...
                            // Step 1: Add non-duplicate items to debitnote_products
                            for (var entry in debitnote_productDetails) {
                              // final isDuplicate = debitnote_products.any((product) => product.productName == entry['productName'] && product.hsn == entry['hsn'] && product.gst == entry['gst'] && product.price == entry['price'] && product.quantity == entry['quantity']);

                              // if (!isDuplicate) {
                              final index = debitnote_products.length + 1; // Generate serial number
                              debitnote_products.add(Product(index.toString(), entry['productName'] as String, entry['hsn'] as String, entry['gst'] as double, entry['price'] as double, entry['quantity'] as int, entry['remarks'] as String));
                              // }
                            }

                            // Step 2: Group products by GST and calculate total for each group
                            debitnote_gstTotals = debitnote_products
