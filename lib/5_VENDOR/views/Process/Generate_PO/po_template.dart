...
Future<Uint8List> generate_PO(PdfPageFormat pageFormat, products, client_addr_name, client_addr, bill_addr_name, bill_addr, po_num, GSTIN, po_gstTotals, isGST_local) async {
  final po = PO(
    products: products,
    ...
    GSTIN: GSTIN,
    type: '',
    po_gstTotals: po_gstTotals,
    isGST_local: isGST_local,
  );

  return await po.buildPdf(pageFormat);
}
...
class PO {
  ...
  double get CGST_total => po_gstTotals.map((item) => (item.gst) / 2 * (item.total) / 100).reduce((a, b) => a + b);
  double get SGST_total => po_gstTotals.map((item) => (item.gst) / 2 * (item.total) / 100).reduce((a, b) => a + b);
  ...
  Future<Uint8List> buildPdf(PdfPageFormat pageFormat) async {
    poController.update_poAmount(_grandTotal);
    poController.update_poSubTotal(_grandTotal);

    if (isGST_local) {
      poController.update_poCGSTAmount(CGST_total);
      poController.update_poSGSTAmount(SGST_total);
      poController.update_poIGSTAmount(0.0);
    } else {
      poController.update_poCGSTAmount(0.0);
      poController.update_poSGSTAmount(0.0);
      poController.update_poIGSTAmount(CGST_total * 2);
    }
            ...
            pw.Container(
              ...
              child: pw.Column(
                ...
                children: [
                  ...
                  pw.ListView.builder(
                    itemCount: po_gstTotals.length, // Number of items in the list
                      ...
                      return pw.Row(
                        children: [
                          pw.Container(
                            ...
                            width: 80,
                            height: 38,
                            child: pw.Center(child: regular(formatzero(po_gstTotals[index].total), 10)),
                          ),
                          pw.Container(
                            height: 38,
                            child: pw.Row(
                              children: [
                                pw.Container(
                                  ...
                                  width: 50, // Define width instead of Expanded
                                  child: pw.Center(
                                    child: regular((po_gstTotals[index].gst).toString(), 10),
                                  ),
                                ),
                                pw.Container(
                                  width: 100, // Define width instead of Expanded
                                  ...
                                  child: pw.Center(
                                    child: regular(
                                        formatzero(
                                          ((po_gstTotals[index].total.toInt() / 100) * (po_gstTotals[index].gst)),
                                        ),
                                        10),
                                  ),
                                ),
                            ...
                            child: pw.Row(
                              children: [
                                pw.Container(
                                  ...
                                  width: 40, // Define width instead of Expanded
                                  child: pw.Center(
                                    child: regular((po_gstTotals[index].gst / 2).toString(), 10),
                                  ),
                                ),
                                pw.Container(
                                  width: 70, // Define width instead of Expanded
                                  ...
                                  child: pw.Center(
                                    child: regular(
                                        formatzero(
                                          ((po_gstTotals[index].total.toInt() / 100) * (po_gstTotals[index].gst / 2)),
                                        ),
                                        10),
                                  ),
                                ),
                            ...
                            child: pw.Row(
                              children: [
                                pw.Container(
                                  ...
                                  width: 40, // Define width instead of Expanded
                                  child: pw.Center(child: regular((po_gstTotals[index].gst / 2).toString(), 10)),
                                ),
                                pw.Container(
                                  width: 70, // Define width instead of Expanded
                                  decoration: const pw.BoxDecoration(
                                  ...
                                  child: pw.Center(child: regular(formatzero(((po_gstTotals[index].total.toInt() / 100) * (po_gstTotals[index].gst / 2))), 10)),
                                ),
                              ],
                            ),
                          ),
    ...
    return pw.Container(
      ...
      child: pw.Column(
        ...
        children: [
          ...
          ...List.generate(poController.poModel.PO_noteList.length, (index) {
            return pw.Padding(
              ...
              child: pw.Row(
                ...
                children: [
                  ...
                  pw.Expanded(
                    child: pw.Text(
                      poController.poModel.PO_noteList[index],
                      textAlign: pw.TextAlign.start,
                      style: pw.TextStyle(
                        font: Helvetica,
                        fontSize: 10,
          ...
          pw.Padding(
            ...
            child: pw.Row(
              crossAxisAlignment: pw.CrossAxisAlignment.start,
              children: [
                regular("${poController.poModel.PO_noteList.length + 1}.", 10),
                pw.SizedBox(width: 5),
                pw.Expanded(
                  child: pw.Column(
                    crossAxisAlignment: pw.CrossAxisAlignment.start,
          ...
          if (poController.poModel.PO_recommendationList.isNotEmpty)
            pw.Padding(
              padding: const pw.EdgeInsets.only(left: 0, top: 5),
              child: pw.Row(
                crossAxisAlignment: pw.CrossAxisAlignment.start,
                children: [
                  regular("${poController.poModel.PO_noteList.length + 2}.", 10),
                  pw.SizedBox(width: 5),
                  pw.Expanded(
                    child: pw.Column(
                      crossAxisAlignment: pw.CrossAxisAlignment.start,
                      children: [
                        bold(poController.poModel.recommendationHeadingController.value.text, 10),
                        ...poController.poModel.PO_recommendationList.map((recommendation) {
                          return pw.Padding(
