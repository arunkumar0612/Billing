...
Future<Uint8List> generate_Debit(PdfPageFormat pageFormat, products, client_addr_name, client_addr, bill_addr_name, bill_addr, Debit_num, gst, Debit_gstTotals) async {
  final Debit = Debit_generate(
    ...
    bill_addr: bill_addr,
    Debit: Debit_num ?? "",
    Debit_gstTotals: Debit_gstTotals,
    // title_text: title,
    type: '',
  );

  return await Debit.buildPdf(pageFormat);
}

class Debit_generate {
  Debit_generate(
  ...
  // String title_text = "";
  String type = "";
  List<Map<String, dynamic>> Debit_gstTotals = [];

  final List<DebitProduct> products;
  final double GST;
  final PdfColor baseColor;
  final PdfColor accentColor;
  static const _darkColor = PdfColors.blueGrey800;
  double get CGST_total => Debit_gstTotals.map((item) => (item['gst'] as double) / 2 * (item['total'] as double) / 100).reduce((a, b) => a + b);
  double get SGST_total => Debit_gstTotals.map((item) => (item['gst'] as double) / 2 * (item['total'] as double) / 100).reduce((a, b) => a + b);
            ...
            pw.Container(
              ...
              child: pw.Column(
                ...
                children: [
                  ...
                  pw.ListView.builder(
                    itemCount: debitController.debitModel.Debit_gstTotals.length, // Number of items in the list
                      ...
                      return pw.Row(
                        children: [
                          pw.Container(
                            ...
                            width: 80,
                            height: 38,
                            child: pw.Center(child: regular(formatzero(Debit_gstTotals[index]['total']), 10)),
                          ),
                          pw.Container(
                            height: 38,
                            child: pw.Row(
                              children: [
                                pw.Container(
                                  ...
                                  width: 40, // Define width instead of Expanded
                                  child: pw.Center(
                                    child: regular((Debit_gstTotals[index]['gst'] / 2).toString(), 10),
                                  ),
                                ),
                                pw.Container(
                                  width: 70, // Define width instead of Expanded
                                  ...
                                  child: pw.Center(
                                    child: regular(
                                        formatzero(
                                          ((Debit_gstTotals[index]['total'].toInt() / 100) * (Debit_gstTotals[index]['gst'] / 2)),
                                        ),
                                        10),
                                  ),
                                ),
                            ...
                            child: pw.Row(
                              children: [
                                pw.Container(
                                  ...
                                  width: 40, // Define width instead of Expanded
                                  child: pw.Center(child: regular((Debit_gstTotals[index]['gst'] / 2).toString(), 10)),
                                ),
                                pw.Container(
                                  width: 70, // Define width instead of Expanded
                                  decoration: const pw.BoxDecoration(
                                  ...
                                  child: pw.Center(child: regular(formatzero(((Debit_gstTotals[index]['total'].toInt() / 100) * (Debit_gstTotals[index]['gst'] / 2))), 10)),
                                ),
                              ],
                            ),
                          ),
    ...
    return pw.Container(
      ...
      child: pw.Column(
        ...
        children: [
          ...
          ...List.generate(debitController.debitModel.Debit_noteList.length, (index) {
            return pw.Padding(
              ...
              child: pw.Row(
                ...
                children: [
                  ...
                  pw.Expanded(
                    child: pw.Text(
                      debitController.debitModel.Debit_noteList[index].notename,
                      textAlign: pw.TextAlign.start,
                      style: pw.TextStyle(
                        font: Helvetica,
                        fontSize: 10,
          ...
          pw.Padding(
            ...
            child: pw.Row(
              crossAxisAlignment: pw.CrossAxisAlignment.start,
              children: [
                regular("${debitController.debitModel.Debit_noteList.length + 1}.", 10),
                pw.SizedBox(width: 5),
                pw.Expanded(
                  child: pw.Column(
                    crossAxisAlignment: pw.CrossAxisAlignment.start,
          ...
          if (debitController.debitModel.Debit_recommendationList.isNotEmpty)
            pw.Padding(
              padding: const pw.EdgeInsets.only(left: 0, top: 5),
              child: pw.Row(
                crossAxisAlignment: pw.CrossAxisAlignment.start,
                children: [
                  regular("${debitController.debitModel.Debit_noteList.length + 2}.", 10),
                  pw.SizedBox(width: 5),
                  pw.Expanded(
                    child: pw.Column(
                      crossAxisAlignment: pw.CrossAxisAlignment.start,
                      children: [
                        bold(debitController.debitModel.Debit_table_heading.value, 10),
                        ...debitController.debitModel.Debit_recommendationList.map((recommendation) {
                          return pw.Padding(
