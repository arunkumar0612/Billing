...
class QuotationNote extends StatefulWidget {
  const QuotationNote({super.key});

  @override
  State<QuotationNote> createState() => _QuotationNoteState();
}

class _QuotationNoteState extends State<QuotationNote> {
  ...
  void _addtable_row() {
    quote_table_heading = TableHeadingController.text;
    // if (_notetable_formKey.currentState?.validate() ?? false) {
    // Check if note Name already exists
    bool exists = quote_recommendationList.any((note) => note['key'] == tablekey_Controller.text);
    if (exists) {
    ...
    setState(
      () {
        ...
        notetablelength = quote_recommendationList.length;
        _cleartable_Fields();
      },
    );
    // }
    ...
    if (_noteformKey.currentState?.validate() ?? false) {
      setState(
        () {
          ...
          _clearnoteFields();
          noteeditIndex = null;
          notelength = quote_noteList.length;
        },
      );
    }
  }
  ...
  void _updatetable() {
    // if (_notetable_formKey.currentState?.validate() ?? false) {
    quote_recommendationList[notetable_editIndex!] = {
      // 'notetype': selectedheadingType ?? '',
      // 'noteheading': selectedheadingType == 'With Heading' ? NoteHeadingController.text : '',
  ...
  void _editnote(int index) {
    Map<String, dynamic> note = quote_noteList[index];
    setState(
      () {
  ...
  void _editnotetable(int index) {
    Map<String, dynamic> note = quote_recommendationList[index];
    setState(
      () {
        // NoteHeadingController.text = note['noteheading'] ?? '';
  ...
  void _addNotes() {
    if (_noteformKey.currentState?.validate() ?? false) {
      // Check if note Name already exists
      bool exists = quote_noteList.any((note) => note['notename'] == notecontentController.text);
      if (exists) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            backgroundColor: Colors.blue,
  ...
  Widget noteLists() {
    return ListView.builder(
        itemCount: quote_noteList.length,
                    ...
                    Expanded(
                      child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              Text(
                                '${quote_noteList[index]['notecontent']}', // Display camera type from map
  ...
  Widget noteTable() {
    return ListView.builder(
        itemCount: quote_recommendationList.length,
        itemBuilder: (context, index) {
          return GestureDetector(
            onTap: () {
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              Text(
                                '${quote_recommendationList[index]['key']}', // Display camera type from map
                                style: const TextStyle(color: Primary_colors.Color1, fontSize: 10),
                              ),
                            ],
                          )),
                    ...
                    IconButton(
                      onPressed: () {
                        setState(() {
                          quote_recommendationList.removeAt(index);
                          notetablelength = quote_recommendationList.length;
                        });
                      },
                      icon: const Icon(
  ...
  Widget build(BuildContext context) {
    quote_recommendationList.isEmpty ? TableHeadingController.clear() : null;
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 10),
            if (quote_recommendationList.isNotEmpty)
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
              ...
              child: Row(
                ...
                children: [
                  Button1(
                    ...
                    onPressed: () {
                      noteeditIndex == null ? GenerateQuotation.backTab() : _resetEditingStateNote(); // Reset editing state when going back
                    },
                  ),
                  if (quote_noteList.isNotEmpty) const SizedBox(width: 10),
                  if (quote_noteList.isNotEmpty || quote_recommendationList.isNotEmpty)
                    Button1(
                      colors: Colors.green,
                      text: 'Submit',
                      onPressed: () async {
                        if (quote_products.isNotEmpty && quote_gstTotals.isNotEmpty && quote_productDetails.isNotEmpty && quote_client_addr_name.isNotEmpty && quote_client_addr.isNotEmpty && quote_bill_addr_name.isNotEmpty && quote_bill_addr.isNotEmpty && quote_title.isNotEmpty) {
                          final pdfData = await generate_Quotation(PdfPageFormat.a4, quote_products, quote_client_addr_name, quote_client_addr, quote_bill_addr_name, quote_bill_addr, quote_estimate_no, quote_title, 9);
                          const filePath = 'E://Quotation.pdf';
