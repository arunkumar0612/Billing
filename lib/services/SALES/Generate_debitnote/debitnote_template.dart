...
List<Product> debitnote_products = [];
List<Map<String, dynamic>> debitnote_gstTotals = [];
//main
String debitnote_client_addr_name = "";
String debitnote_client_addr = "";
String debitnote_bill_addr_name = "";
String debitnote_bill_addr = "";
String debitnote_no = "";
// String debitnote_title = "";
String debitnote_table_heading = "";
List<Map<String, dynamic>> debitnote_noteList = [];
List<Map<String, dynamic>> debitnote_recommendationList = [];
List<Map<String, dynamic>> debitnote_productDetails = [];

//

Future<Uint8List> generate_debitnote(PdfPageFormat pageFormat, products, client_addr_name, client_addr, bill_addr_name, bill_addr, debitnote_num, gst) async {
  final debitnote = Request_for_quote(
    products: products,
    GST: gst.toDouble(),
    ...
    bill_addr: bill_addr,
    debitnote: debitnote_num ?? "",
    // title_text: title,
    type: '',
  );

  return await debitnote.buildPdf(pageFormat);
}
...
class Request_for_quote {
  ...
  double get CGST_total => debitnote_gstTotals.map((item) => (item['gst'] as double) / 2 * (item['total'] as double) / 100).reduce((a, b) => a + b);
  double get SGST_total => debitnote_gstTotals.map((item) => (item['gst'] as double) / 2 * (item['total'] as double) / 100).reduce((a, b) => a + b);
            ...
            pw.Container(
              ...
              child: pw.Column(
                ...
                children: [
                  ...
                  pw.ListView.builder(
                    itemCount: debitnote_gstTotals.length, // Number of items in the list
                      ...
                      return pw.Row(
                        children: [
                          pw.Container(
                            ...
                            width: 80,
                            height: 38,
                            child: pw.Center(child: regular(formatzero(debitnote_gstTotals[index]['total']), 10)),
                          ),
                          pw.Container(
                            height: 38,
                            child: pw.Row(
                              children: [
                                pw.Container(
                                  ...
                                  width: 40, // Define width instead of Expanded
                                  child: pw.Center(
                                    child: regular((debitnote_gstTotals[index]['gst'] / 2).toString(), 10),
                                  ),
                                ),
                                pw.Container(
                                  width: 70, // Define width instead of Expanded
                                  ...
                                  child: pw.Center(
                                    child: regular(
                                        formatzero(
                                          ((debitnote_gstTotals[index]['total'].toInt() / 100) * (debitnote_gstTotals[index]['gst'] / 2)),
                                        ),
                                        10),
                                  ),
                                ),
                            ...
                            child: pw.Row(
                              children: [
                                pw.Container(
                                  ...
                                  width: 40, // Define width instead of Expanded
                                  child: pw.Center(child: regular((debitnote_gstTotals[index]['gst'] / 2).toString(), 10)),
                                ),
                                pw.Container(
                                  width: 70, // Define width instead of Expanded
                                  decoration: const pw.BoxDecoration(
                                  ...
                                  child: pw.Center(child: regular(formatzero(((debitnote_gstTotals[index]['total'].toInt() / 100) * (debitnote_gstTotals[index]['gst'] / 2))), 10)),
                                ),
                              ],
                            ),
                          ),
    ...
    return pw.Container(
      ...
      child: pw.Column(
        ...
        children: [
          ...
          ...List.generate(debitnote_noteList.length, (index) {
            return pw.Padding(
              ...
              child: pw.Row(
                ...
                children: [
                  ...
                  pw.Expanded(
                    child: pw.Text(
                      debitnote_noteList[index]["notecontent"],
                      textAlign: pw.TextAlign.start,
                      style: pw.TextStyle(
                        font: Helvetica,
                        fontSize: 10,
          ...
          pw.Padding(
            ...
            child: pw.Row(
              crossAxisAlignment: pw.CrossAxisAlignment.start,
              children: [
                regular("${debitnote_noteList.length + 1}.", 10),
                pw.SizedBox(width: 5),
                pw.Expanded(
                  child: pw.Column(
                    crossAxisAlignment: pw.CrossAxisAlignment.start,
          ...
          if (debitnote_recommendationList.isNotEmpty)
            pw.Padding(
              padding: const pw.EdgeInsets.only(left: 0, top: 5),
              child: pw.Row(
                crossAxisAlignment: pw.CrossAxisAlignment.start,
                children: [
                  regular("${debitnote_noteList.length + 2}.", 10),
                  pw.SizedBox(width: 5),
                  pw.Expanded(
                    child: pw.Column(
                      crossAxisAlignment: pw.CrossAxisAlignment.start,
                      children: [
                        bold(debitnote_table_heading, 10),
                        ...debitnote_recommendationList.map((recommendation) {
                          return pw.Padding(
