...
class sub_invoiceProducts extends StatefulWidget {
  const sub_invoiceProducts({super.key});

  @override
  State<sub_invoiceProducts> createState() => _sub_invoiceProductsState();
}

class _sub_invoiceProductsState extends State<sub_invoiceProducts> {
  int? editIndex1;
  ...
  void _addproduct() {
    if (formKey1.currentState?.validate() ?? false) {
      ...
      bool exists = sub_invoice_productDetails.any((product) => product['productName'] == productNameController.text && product['hsn'] == hsnController.text && product['price'] == double.parse(priceController.text) && product['quantity'] == int.parse(quantityController.text) && product['gst'] == double.parse(GSTController.text));
      ...
      setState(() {
        sub_invoice_productDetails.add({
        ...
        _clearFields();
        // print(sub_invoice_productDetails.length);
      });
    }
  }
  ...
  void _updateproduct() {
    if (formKey1.currentState?.validate() ?? false) {
      setState(
        () {
          sub_invoice_productDetails[editIndex1!] = {
            'productName': productNameController.text,
            'hsn': hsnController.text,
            'price': double.parse(priceController.text),
            'quantity': int.parse(quantityController.text),
  ...
  void _editproduct(int index) {
    Map<String, dynamic> product = sub_invoice_productDetails[index];
    setState(
      () {
  ...
  Widget sub_invoice_productDetailss() {
    return Column(
      mainAxisAlignment: MainAxisAlignment.start,
      crossAxisAlignment: CrossAxisAlignment.center,
      children: [
        for (int i = 0; i < sub_invoice_productDetails.length; i++)
          Column(
            mainAxisAlignment: MainAxisAlignment.start,
                        ...
                        SizedBox(
                          ...
                          child: Padding(
                            ...
                            child: Text(
                              ...
                              '${i + 1}. ${sub_invoice_productDetails[i]['productName']}', // Display camera type from map
                        ...
                        Row(
                          children: [
                            IconButton(
                              onPressed: () {
                                setState(() {
                                  sub_invoice_productDetails.removeAt(i);
                                });
                              },
                              icon: const Icon(
                                Icons.close,
                    ...
                    Row(
                      ...
                      children: [
                        Button1(
                          ...
                          onPressed: () {
                            editIndex1 == null ? Generatesub_invoice.backTab() : _resetEditingState(); // Reset editing state when going back
                          },
                        ),
                                ...
                                child: Padding(
                                  ...
                                  child: SingleChildScrollView(
                                    child: Padding(
                                      ...
                                      child: sub_invoice_productDetailss(),
                                    ),
                                  ),
                                ),
                              ),
                      ...
                      if (sub_invoice_productDetails.isNotEmpty)
                        Button1(
                          ...
                          onPressed: () {
                            ...
                            // Step 1: Add non-duplicate items to sub_invoice_products
                            for (var entry in sub_invoice_productDetails) {
                              // final isDuplicate = sub_invoice_products.any((product) => product.productName == entry['productName'] && product.hsn == entry['hsn'] && product.gst == entry['gst'] && product.price == entry['price'] && product.quantity == entry['quantity']);

                              // if (!isDuplicate) {
                              final index = sub_invoice_products.length + 1; // Generate serial number
                              sub_invoice_products.add(Product(
                                index.toString(),
                            ...
                            sub_invoice_gstTotals = sub_invoice_products
