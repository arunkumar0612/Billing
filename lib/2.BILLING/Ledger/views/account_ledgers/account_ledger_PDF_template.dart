...
Future<Uint8List> generateInvoice(
  ...
  final clientDetails = ClientDetails(
    ...
    clientAddress:
        '131b, Raja Street, Rivoli 75001 Paris, Palani Road, Rivoli 75001 Paris, Palani Road, Rivoli 75001 Paris, France',
  ...
  final temp = {
    ...
    'ledgerDetails': parsedLedgerList
  };
...
pw.Widget _buildFooter(pw.Context context) {
  ...
  pw.PageTheme _buildTheme(PdfPageFormat pageFormat) {
    return pw.PageTheme(
      margin:
          const pw.EdgeInsets.only(top: 30, bottom: 20, right: 15, left: 15),
    ...
    (index) {
      ...
      return pw.TableRow(
        children: [
          ...
          _buildCell(
            _formatCurrency(parseBalanceWithSuffix(item.balance).abs()) +
              (item.balance.toLowerCase().contains('dr') ? ' Dr' : ' Cr'),
            alignRight: true,
          ),
        ],
      );
...
double parseBalanceWithSuffix(String balance) {
  ...
  pw.Widget totalRow() {
  final totalDebit = clientLedger.ledgerDetails.fold<double>(0.0,(sum, item) => sum + (double.tryParse(item.debit) ?? 0),);
  ...
  pw.Widget _builddescription(String text, String Sub_text,
      {bool isDescription = false, bool alignRight = false}) {
    return pw.Container(
        padding:
            const pw.EdgeInsets.only(bottom: 10, top: 10, left: 5, right: 5),
        alignment: isDescription
            ? pw.Alignment.topLeft
